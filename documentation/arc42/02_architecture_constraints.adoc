...
|The canonical hierarchy is `Platform -> Tenant -> Workspace -> Project / Service -> Environment`.
...
|Authorization model
|Access must follow `Principal -> Membership -> Grant -> Role -> Permission -> Scope`.
...
|The maintained source must stay in AsciiDoc, C4 diagrams must come from `documentation/workspace.dsl`, and ADR updates must flow through `exportMarkdown` before HTML generation.
|===
