...
|The architecture now fixes the audit storage direction, but it still leaves runtime platform, exact state-integration technology, and network implementation open, which can cause teams to make divergent assumptions.
|Capture the remaining concrete infrastructure decisions in future ADRs once those choices are made and align sections 6 and 7 accordingly.

|6
|R-06
|Public access completion
|The public interaction model is now fixed, but the identity/token decision and browser-composition decision remain open, which can still create uncertainty at the boundary between public access, trust, and browser behavior.
|Use ADR 0008 as the fixed public interface baseline, then resolve ADR 0009 for trust and credentials and ADR 0013 for browser-composition responsibilities without changing that public interaction model.
...
|link:../adrs/0009-select-identity-federation-and-service-account-credential-model.adoc[ADR 0009], Sections 3 and 8
|The platform gains one concrete trust and token model for users and service accounts.

|A-03
|Resolve the authoritative state integration and projection technology profile.
|link:../adrs/0011-select-authoritative-state-integration-and-projection-technology-profile.adoc[ADR 0011], Sections 6, 7, and 8
