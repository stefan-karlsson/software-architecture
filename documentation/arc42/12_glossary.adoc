...
|Platform Hierarchy
|Shared tenant, workspace, workload, and environment structure used to interpret access, policy, audit context, and defaults across the platform.
...
|Audit Record Store
|Append-only audit store that keeps the official record for audit records and supports governance retrieval and review.

|Audit Record Archive
|Immutable object-storage copy of audit records used for long-term retention and recovery.
...
|Browser unit a feature team ships independently and a tenant app or administration app composes through host-defined integration boundaries.

|Platform Management API
|Versioned public API used by browser applications and automation clients to perform governed platform actions.
...
|Runtime Unit
|Isolated runtime unit used to host shared or dedicated environments without changing the underlying business hierarchy.
