...
* the boundary between authoritative evidence and operational telemetry
...
* The platform uses a dedicated audit store as the authoritative system of record for audit evidence.
...
* Audit retention is enforced by the authoritative audit store according to the combined effective policy and mandatory platform baseline controls.
...
* Audit review and retention enforcement must operate from authoritative audit evidence rather than observability copies.
...
* Loss of observability copies must not imply loss of authoritative audit evidence.
...
* Exported telemetry should preserve correlation identifiers that allow operators and auditors to trace operational signals back to authoritative audit records.

### Normative Commit and Integrity Rules

* Privileged or regulated actions must not be treated as successfully committed until authoritative audit evidence has been durably accepted by the audit architecture.
...
* If authoritative audit evidence for a privileged or regulated mutation cannot be durably accepted, the platform must not report the mutation as successfully committed.
...
* Authoritative audit records must preserve at minimum: event identifier, event timestamp, actor identity, actor type, action or event type, target scope, scope path, decision or outcome, source capability, correlation identifier, and retention context.
...
* Audit review, compliance export, and governance evidence retrieval must read from the authoritative audit store.
* Operational dashboards, alerts, and troubleshooting flows may read from observability exports.
* Export to observability may summarize or redact data for operational use, but it must not replace authoritative evidence retention.
...
* Privileged workflows may need to fail or remain pending when authoritative audit evidence cannot be durably accepted.
* Teams must manage correlation between authoritative audit records and operational telemetry instead of assuming they are the same artifact.
...
* Section 5 reflects that Audit Evidence owns authoritative audit evidence rather than only telemetry export.
* Section 7 keeps concrete storage technology open while describing the authoritative audit store as a required architectural concern.
* Section 10 includes scenarios that distinguish authoritative evidence retention from observability exports and cover commit failure when audit evidence cannot be durably accepted.
* Section 11 treats missing authoritative audit persistence as a material implementation risk until concrete storage choices are made.
...
* A membership change creates an authoritative audit record with actor, scope, action, and resulting effect context.
* A policy-driven denial, such as blocked secret access from an untrusted network, creates an authoritative audit record that preserves permission context, policy outcome, and final decision.
* A production deployment approval flow records request, approval decision, execution outcome, and relevant scope chain.
* Effective policy requires 365-day retention, and the authoritative audit store preserves the evidence for that duration.
...
* The observability stack is unavailable while the authoritative audit store is available, and governed actions can still complete with authoritative evidence preserved.
...
* An operator correlates an observability event back to the authoritative audit record through a shared correlation identifier.
...
* Good, because authoritative evidence remains durable and trustworthy.
...
* Bad, because the platform must operate both authoritative evidence storage and telemetry export flows.

### Use a mixed model with audit-grade storage plus operational summaries

* Good, because it acknowledges different audiences and use cases.
* Bad, because without a clear source-of-truth rule it can still become ambiguous which records are authoritative.
